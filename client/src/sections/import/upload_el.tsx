...
import { mime_type, upload_start } from "./uploader";
import Swal from "sweetalert2";

export default function Upload($: { onUpload: (url: string) => void }) {
  const [file, setFile] = useState<File | null>(null);
  const [wip, set_wip] = useState(false);

  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const selectedFile = event.target.files?.[0];
    if (selectedFile && selectedFile.type === mime_type) {
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    if (droppedFile && droppedFile.type === mime_type) {
      setFile(droppedFile);
    } else {
      alert("Пожалуйста, выберите XLSX файл.");
    }
  ...
  const start_upload = async () => {
    ...
    try {
      ...
      $.onUpload(url);
      await Swal.fire("Успешно", "Данные загружены. Нажмите ОК для обновления страницы", "success");
      location.hash = "";
      location.reload();
      setFile(null);
    } catch (e) {
  ...
  return (
    <Card className="w-full">
      <CardContent className="p-6">
        <h1 className="text-2xl font-bold text-center mb-6">Загрузка данных в формате XLSX</h1>
        <div
        ...
        >
          {file ? (
            <div className="space-y-2">
              <i className="bx bxs-file-blank text-5xl text-blue-500"></i>
              <p className="text-sm text-gray-600">{file.name}</p>
            </div>
          ) : (
            <div className="space-y-4">
              <i className="bx bx-upload text-5xl text-gray-400"></i>
              <p className="text-gray-600">Перетащите XLSX файл сюда или кликните для выбора</p>
            </div>
          )}
          <input id="fileInput" type="file" accept=".xlsx" className="hidden" onChange={handleFileChange} />
        </div>
        {file && (
